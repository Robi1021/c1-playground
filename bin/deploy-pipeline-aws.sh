...
cat <<EOF >$PGPATH/pipeline-aws-down.sh
set -e
EOF
chmod +x $PGPATH/pipeline-aws-down.sh
...
function create_iam_role_eks() {
  ...
  if [ -z "${CODEBUILD_ROLE_NAME}" ]; then
    ...
    echo "aws iam delete-role-policy --role-name ${CODEBUILD_ROLE_NAME} --policy-name eks-describe-ecr-pull" >> $PGPATH/pipeline-aws-down.sh
    echo "aws iam delete-role --role-name ${CODEBUILD_ROLE_NAME}" >> $PGPATH/pipeline-aws-down.sh
  else
...
function deploy_cloudformation() {
  ...
  if [ -z $(aws cloudformation describe-stacks --output json --region eu-central-1 | jq -r --arg app "${APP_NAME}" '.Stacks[] | select(.StackName | contains($app)) | .StackName') ]; then
    ...
    echo "aws ecr delete-repository --repository-name ${APP_NAME} --force" >> $PGPATH/pipeline-aws-down.sh
    echo "aws cloudformation delete-stack --stack-name ${APP_NAME}-pipeline" >> $PGPATH/pipeline-aws-down.sh
  else
