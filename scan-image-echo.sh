...
echo -e "\e[1;41mecho ${REG_PASSWORD} | docker login ${REGISTRY} --username ${REG_USERNAME} --password-stdin\e[1;m"
  echo ${REG_PASSWORD} | docker login ${REGISTRY} --username ${REG_USERNAME} --password-stdin

echo -e "\e[1;41mdocker pull ${TARGET_IMAGE}\e[1;m"
  docker pull ${TARGET_IMAGE}

  # Tag and push, but strip hash
echo -e "\e[1;41mdocker tag ${TARGET_IMAGE} ${REGISTRY}/${TARGET_IMAGE%@*}\e[1;m" 
  docker tag ${TARGET_IMAGE} ${REGISTRY}/${TARGET_IMAGE%@*}

echo -e "\e[1;41mdocker push ${REGISTRY}/${TARGET_IMAGE%@*}\e[1;m" 
